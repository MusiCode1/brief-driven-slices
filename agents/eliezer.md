...
description: >
  ...
  manual / none), not assumed. Sonnet-level. Stops and reports when
  stuck (does NOT try to solve architectural problems on its own).
  Invokes calev (runtime-verifier) after UI/data-flow/route phases.
  Invokes calev at the end of each slice.
  ...
  The prompt must include: brief path, slice name, environment notes
  (which ports run what, where the browser is, what the test fixtures are).
mode: all
...
- [ ] **‏Walkthrough ‏מעודכן** — ‏לפי הסקיל `update-walkthrough` (‏רשומה ב-`docs/walkthrough.md` ‏עם מה בוצע, ‏החלטות, ‏בדיקות)
