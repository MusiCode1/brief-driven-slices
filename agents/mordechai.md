...
>
> 🔴 **ההמתנה מוחזקת אך ורק ע"י `$BDS_SCRIPTS/watch-dispatch.sh`** (ב-tmux,
> עם `--notify-agent <המזהה-שלך>`) — אסור לכתוב צופה ad-hoc ואסור לולאת-סנטינל
> ידנית. הוא מעיר אותך בהכרעה (exit 0/3, לפי ארטיפקט) או בחשד-קיפאון.
> `notify` מהילד הוא קיצור-דרך, לא תחליף — ההכרעה תמיד לפי git. פער 9.
