...
1. ‏הרץ `python3 <bds>/scripts/cleanup_state.py <project>`
2. ‏קרא `~/.local/state/brief-driven-slices/<project>/state.json`
3. ‏פעל לפי הלולאה ב-`<bds>/agents/yetro.md`
...
`<bds>` = ‏שורש ה-checkout ‏שלך ‏של brief-driven-slices (‏הנתיב הקונקרטי-למכונה **‏לא**
‏חי כאן — ‏הוא ‏ב-`paths.env` ‏שלך, ‏ראה `cli-configs/paths.env.example` ‏ב-`<bds>`):
