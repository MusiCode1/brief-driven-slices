...
**‏מנגנון ה — ‏השער אינו בר-הרצה על התשתית הקיימת** (‏נוסף ‏מריצה 14, 2026-08-25):
‏שורת-DoD ‏דרשה "‏טסט שמרכיב את רכיב-הבועה ‏ומאמת שהתרשים מופיע **‏בבועת הסוכן**".
‏היא נראתה מצוינת — ‏ממוקדת, ‏עם עמודת-כישלון מפורשת, ‏ונוקבת ברכיב בשמו.
‏אלא ש-`mount()` ‏על רכיב Svelte **‏נכשל בתצורת-הטסטים של אותו ריפו**
(`lifecycle_function_unavailable`), ‏ו**‏לא היה ולו תקדים אחד** ‏להרכבת רכיב בטסט.

‏זה שונה ממנגנונים א–ד: ‏השער לא "‏עובר תמיד" ‏ולא "‏לא מבחין" — ‏הוא פשוט **‏לא רץ**.
‏והתוצאה הצפויה גרועה במיוחד: ‏המבצע נתקע 30–60 ‏דק', ‏ואז **‏מחליש את השער** ‏למשהו
‏שכן רץ — ‏בדיוק המשטח שה-DoD ‏נכתב כדי לא לבדוק.

**‏הגנה** — ‏הרחבה של "‏להריץ כל שער על הבסיס":
> ‏לא מספיק לשאול "‏מה יגרום לזה להיכשל". ‏חובה לשאול גם: **"‏האם אני יכול להריץ
> ‏את השורה הזו **‏עכשיו**, ‏על הבסיס, ‏כמו שהוא?"** ‏אם התשובה כרוכה בשינוי תשתית —
> ‏**‏זה קומיט בסלייס, ‏לא הערת-שוליים**, ‏והוא נכנס ל-scope ‏ולציון-המורכבות.

‏בריצה 14 ‏המאמתת מדדה את הפתרון (‏שורה אחת ב-`vitest.config.ts` + ‏רתמת-קונטקסטים,
‏סוויטה מלאה ירוקה) ‏והוא הפך ל-Commit 0ב. ‏ההפרש בין "‏ממצא" ‏ל"‏מבוי-סתום" ‏היה
‏**‏שהמאמתת הריצה, ‏לא ששיערה**.
...
## קטגוריה 13: `exec-only-hostile-gate` — ‏השער העוין בדק הרצת-קוד, ‏ושכח בקשת-רשת

**‏מקור**: ‏ריצה 14 (`msg-diagrams`, drive-coding, 2026-08-25) — ‏ממצא חוסם של אביגיל.

‏סלייס הכניס allowlist ‏חדש ל-SVG ‏שמייצרת ספריית-תרשימים, ‏והתיר בו `style` ‏ו-`<style>`
‏בנימוק המקובל: *"‏ה-input ‏הוא פלט של **‏גנרטור**, ‏לא HTML ‏מהמודל"* — ‏בדיוק הנימוק
‏שכבר היה בקוד עבור KaTeX.

**‏הנימוק היה שגוי חלקית, ‏וזה מה שהופך אותו לתבנית:** ‏פלט-הגנרטור ‏**‏נשא בתוכו
‏מחרוזת שהמודל שלט בה** — ‏הספרייה מכבדת הוראה בתוך המקור (`themeCSS`) ‏שמזריקה
‏CSS ‏חופשי ל-`<style>` ‏המוטמע. ‏משם `url(http://…)` ‏**‏יורה בקשת-רשת אמיתית**
(‏נמדד: ‏בקשות שהגיעו לשרת מאזין) — ‏ערוץ-דליפה מהודעת-סוכן.

**‏למה כל השערים היו ירוקים:** ‏גם הספייק ‏וגם ה-DoD ‏העוין בדקו את מה שבודקים תמיד —
`<script`, `on*=`, `javascript:`, `foreignObject`. ‏כלומר **‏הרצת-קוד**.
‏אף אחד לא בדק `url(`. ‏הערוץ היה עובר ל-merge ‏ירוק לגמרי.

**‏הגנה**:
- ‏כל allowlist ‏שמתיר `style=` ‏או `<style>` ‏הוא **‏ערוץ-רשת**, ‏לא רק ‏משטח-XSS.
  ‏השער העוין חייב לכלול: `url(`, `@import` (‏גם ‏בצורת-מחרוזת, ‏בלי `url()`), `image-set(`.
- **‏"‏פלט של גנרטור" ‏אינו חסינות.** ‏לשאול: ‏האם הגנרטור מקבל **‏הנחיות** ‏מהמקור
  ‏שהמודל כתב? ‏אם כן — ‏חלק מהפלט הוא ‏קלט-משתמש בתחפושת.
- ‏שתי שכבות, ‏כי הן חוסמות דברים שונים: ‏חסימה **‏בגנרטור** (‏רשימת מפתחות שהוראה
  ‏אינה יכולה לדרוס) ‏חוסמת גם את הבקשה שיוצאת **‏בזמן הרינדור עצמו**, ‏לפני שה-sanitize
  ‏שלנו בכלל רץ; ‏חסימה ב-sanitize ‏מכסה מסלולים עתידיים.
- ⚠️ ‏ובאותה נשימה — ‏שכבת-ה-sanitize ‏בפועל כיסתה **‏רק** `url(...)` ‏בתוך `<style>`,
  ‏ולא ‏תכונת `style=`. ‏זה התגלה **‏רק כשהמאמת הריץ את הפונקציה עצמה** ‏מול מחרוזות
  ‏מפוברקות. ‏"‏שכבת-הגנה נוספת" ‏שלא נמדדה ‏היא הנחה, ‏לא שכבה.
