...
`scripts/lint-brief.js`, ~200–300 שורות. **טרם נבנה** — מעקב: [`BACKLOG.md`](BACKLOG.md) שורות 3–4.
...
> ⚠️ **‏המעקב החי עבר ל-[`BACKLOG.md`](BACKLOG.md)** (2026-08-18). ‏הטבלה למטה היא
> **‏תצלום היסטורי** ‏של מה שהסשן ההוא הותיר — ‏אין לעדכן אותה. ‏למצב עדכני:
...
## שיפור מסשן 2026-08-16 (drive-coding — `cli-specs-hot-reload`, 8 סבבים)

### 40. תיקון ממצא הוא **שאילתה**, לא עריכה — `grep` אחרי כל תיקון

**מטפל ב:** ניפוח סבבים שנגרם ע"י המתקן, לא ע"י הבריף. מקור: אביגיל r8 על
`cli-specs-hot-reload`, שאבחנה את התבנית מפורשות אחרי ששאלנו אותה "האם אנחנו
מתכנסים או מסתובבים".

**הבעיה:** הדוח מצטט **אתר אחד** — `§6 שורה 459`. מרדכי מתקן **את השורה הזאת**.
אבל אותה הוראה מופיעה בעוד ארבעה מקומות במסמך, והסבב הבא מוצא אותם:

| ‏מעבר | ‏מה שרד |
|---|---|
| r4 → r5 | ‏ההכרזה "‏החיווט יורד" נוספה כ**הערה**, בעוד ההוראה עצמה נשארה ב-14 מקומות |
| r5 → r6 | ‏האתר ה-14 — ‏שורת-סיכון ב-§6 |
| r6 → r7 | ‏תיקון שנשמר **רק בהודעת ה-commit**; הסקריפט נפל על assertion אחרי שהדפיס ✓ ולפני שכתב |
| r7 → r8 | ‏"‏+layout פותח echo WS" ‏שרד ב-2 מתוך 4 אתרים |

ארבעה סבבים, אותה תבנית. **החומרה יורדת** (17→14→12→11→7) — כלומר זו לא לולאה
אינסופית — אבל כל סבב עולה ~8 דקות אימות ועוד מחזור-תיקון.

**הכשל הקוגניטיבי:** הדוח נקרא כ-**רשימת-מטלות** (`file:line` → תקן), בעוד הוא
**דגימה**. המאמת מצטט את האתר שהוא נתקל בו, לא את קבוצת האתרים.

**הפתרון — שתי שורות אחרי כל תיקון:**

```bash
grep -n "<הביטוי שתוקן>" <brief>     # כל האתרים, לא רק המצוטט
grep -n "<הסימבול שהוסר>" <brief>    # אחרי הסרה — אמת אפס
```

**וכשמריצים סקריפט-עריכה: לאמת אחרי הכתיבה, לא לסמוך על הפלט.** ב-r7 סקריפט
פייתון הדפיס `✓` על תיקון, ואז נפל על `assert` של תיקון אחר — **לפני** ה-`write`.
הפלט אמר שהתיקון בוצע; הקובץ לא השתנה; והודעת ה-commit טענה אותו. המאמת תפס.
כלל: `grep` על הקובץ **אחרי** הכתיבה, לא `print` בתוך הסקריפט.

**מדד הצלחה:** ב-r6, שבו כן הרצנו `grep` אחרי ההסרה, לא נשארה אף שארית של אותו
סימבול. זה הסבב היחיד מתוך הארבעה שלא ייצר ממצא-המשך מאותו סוג.

**קשור:** `plan-pitfalls.md` קטגוריה 12 (`convention-violation`) — שם הפער הוא
מה שהמאמת **לא בודק**; כאן הפער הוא מה שהמתקן **לא מכליל**. שניהם התגלו באותו סשן.
