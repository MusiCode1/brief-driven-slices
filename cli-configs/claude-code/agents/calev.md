...
```
WRONG-DISPATCH · כלב (phase/light) רץ על cursor/composer-2.5 דרך docs/dispatch.md
(MCP, ו-API לגיבוי), לא כתת-סוכן ב-process.
```

**החריג היחיד**: `IN-PROCESS-APPROVED` בגוף ה-prompt — המשגר בחר במודע. המשך.

> אימות-ריצה שרץ על אותו מודל שכתב את הקוד אינו עדות עצמאית. הנחיית-משתמש מ-20/08.
...
1. אם יש כלי `notify_parent` — קרא אותו עם שורת-סיכום (כלב · סלייס · verdict · נתיב דוח).
