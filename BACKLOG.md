...
- [ ] **שער-השורש `typecheck` אינו מכסה את ה-FE** — `tsc --build` בשורש מוגדר עם
      `references` ל-`packages/core` ו-`packages/backend` **בלבד**; ה-FE נבדק
      ב-`svelte-check` (`packages/frontend` script `typecheck`) שאינו רץ משם.
      ⇒ כל `bun run typecheck` ירוק **אינו אומר דבר על קוד FE**.
      *מקור: ריצת `live-voice` 27/08 — חמישה סלייסים, כל סבבי-התיקון, ודיווחי
      "typecheck 0" של המתכנן ושל המבצעים כאחד, לא כיסו FE. נתפס רק כשכלב מדד
      `svelte-check` ישירות ומצא 59 מול 54 בבסיס — הסלייס הוסיף 5, שתיים בקוד-מוצר.*
      ⚠️ **54 השגיאות הקיימות הן סלייס משל עצמן** — התיקון כאן הוא **חיווט השער**,
      לא ניקוי הרשימה.

- [ ] 🔴 **`live-context` מוזג עם אפס צרכני-ייצור** — נמדד 27/08 בענף-ההרצה:
      `buildLiveSeed` · `searchSessionBubbles` · `upsertMemory` · `formatMemoryForPrompt`
      — **לאף אחד מהארבעה אין קורא מחוץ לטסטים שלו** (ל-`buildLiveSeed` יש גם
      קריאה מ-`scripts/probe-live-adapter.mjs`, שהוא חפץ-ריצה ולא מוצר).
      כלומר סלייס שלם נבנה, אומת, קיבל **GO** ומוזג — ואיש אינו קורא לו.
      *הפעולה: לחווט את ה-seed ל-`live.svelte.ts` (הזרקה ב-`connect`), או להצהיר
      את `live-context` כתשתית-מוקדמת ולתעד מי הצרכן המיועד ומתי.*
      ⚠️ **לא לפתוח בתוך סלייס אחר** — הוא קיבל GO ומוזג; פתיחה-מחדש אגב היא
      בדיוק הזחילה שנמנעה כל הריצה.

      🔴 **ההופעה השלישית של אותה פתולוגיה בריצה אחת**: `VoiceMode.state` (קוד מת
      שנמחק) · `mouth` (מוזג בלי צרכן, חוב מוצהר) · `live-context` (סלייס שלם).
      **בכל שלוש השערים היו ירוקים.** ⇒ **שער-GO אינו מוכיח שהקוד נצרך** — הוא
      מוכיח שהוא נכון. זה הדפוס-האח של "שער שאינו יכול להיכשל", וראוי לשער-חיווט
      גנרי (‏"לכל סמל ציבורי חדש — מי קורא לו?").
